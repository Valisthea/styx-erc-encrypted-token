...
author: Kairos (@Valisthea) <contact@kairoslab.xyz>
...
interface IERCXXXX {
    ...
    event BlindTransfer(
        ...
        address indexed to,
        bytes32 proofHash,   // non-indexed: frees 3rd indexed slot (OMEGA L-01)
        uint256 keyVersion   // which key encrypted this transfer
    );
    ...
    /// @notice Emitted when the FHE encryption key is rotated. (OMEGA C-04)
    /// @dev    All clients MUST re-encrypt pending operations with the new key.
    ...
    ///         If confidential, implement the IERCXXXX_ConfidentialSupply extension. (OMEGA C-02)
    ...
    ///         SECURITY: Observers CAN detect ciphertext changes, revealing
    ///         transaction activity without decryption. Use constant-size
    ///         ciphertexts. See Security Considerations. (OMEGA C-03)
    ...
    /// @return The public key bytes (current key version).
    function publicKey() external view returns (bytes memory);

    /// @notice Returns the current encryption key version. (OMEGA C-04)
    function keyVersion() external view returns (uint256);

    /// @notice Returns whether a given key version is still valid. (OMEGA C-04)
    ...
    /// @param  proof            Proof that the allowance ciphertext is valid
    ///                          and encodes a non-negative value.
    ///                          MUST NOT require allowance <= balance.
    ///                          Balance check occurs at blindTransferFrom. (OMEGA H-01)
    ...
    ///         actual encrypted balance.
    ///         SECURITY: proof MUST be generated by `account` or authorized
    ///         delegate and MUST bind to prover identity to prevent
    ///         third-party binary-search probing attacks. (OMEGA M-04)
...
The base `IERCXXXX` is intentionally minimal. Optional capabilities are defined as separate extension interfaces:

**IERCXXXX_ConfidentialSupply** (OMEGA C-02) — for tokens where `totalSupply` is confidential:
```solidity
...
**IERCXXXX_Fees** (OMEGA C-01) — optional fee hooks for FHE computation costs:
```solidity
interface IERCXXXX_Fees is IERCXXXX {
    event FeeCollected(address indexed payer, address indexed recipient, uint256 amount);
    function feeConfig() external view returns (address feeRecipient, uint256 feeBasisPoints, uint256 pGasPerGate);
    function estimateBlindTransferCost() external view returns (uint256 pGas);
}
```

**IERCXXXX_Batch** (OMEGA M-05) — batch blind transfers with a single aggregated proof:
```solidity
...
```
publicInputs = {
    chainId:             uint256,       // EIP-155 chain ID (OMEGA H-03: cross-chain replay)
    contractAddress:     address,       // token contract address (OMEGA H-03)
    from:                address,       // sender
    to:                  address,       // recipient
    encryptedAmountHash: bytes32,       // hash of the encrypted amount
    oldBalanceHashFrom:  bytes32,       // hash of sender pre-transfer encrypted balance
    newBalanceHashFrom:  bytes32,       // hash of sender post-transfer encrypted balance
    oldBalanceHashTo:    bytes32,       // hash of recipient pre-transfer encrypted balance
    newBalanceHashTo:    bytes32,       // hash of recipient post-transfer encrypted balance
    nonce:               uint256        // replay protection (per contract)
}
```
...
```solidity
// IERCXXXX_Shielded Extension (OMEGA L-03)
// PRIVACY WARNING: shield() amount is plaintext and publicly visible.
// For maximum privacy, shield in multiple txs of varying amounts.
...
### Ciphertext Observation Attack (OMEGA C-03)

`encryptedBalanceOf` is a public view function. While observers cannot decrypt the ciphertext, they CAN detect when it changes, revealing that a transfer occurred. This is an accepted metadata leak inherent to public blockchains. Implementations MUST use constant-size ciphertexts (TFHE fixed-width types satisfy this by default) to prevent size-based magnitude leakage.

### Key Compromise and Rotation (OMEGA C-04)

If the FHE secret key is compromised, all encrypted balances are permanently exposed. The `KeyRotated` event, `keyVersion()`, and `isKeyActive()` provide a migration path. Implementations MUST use threshold key management (Shamir Secret Sharing with >= 5 custodians) or HSMs. The STYX Protocol extends this with the Oblivion ceremony (ERC-YYYY: Cryptographic Amnesia Interface) for provable key destruction.
