...
interface IERCXXXX {
    ...
    ///         The contract verifies the proof on-chain and applies the
    ///         homomorphic balance update without decryption.
    /// @param  to               The recipient address.
    ...
    /// @param  proof            Proof that the allowance is valid
...
ERC-20's `Transfer(from, to, amount)` leaks the amount. `BlindTransfer(from, to, proofHash)` preserves the minimum information needed for indexing (who transferred to whom) without revealing how much. The `proofHash` enables off-chain auditors (with appropriate keys) to verify transfer validity.
...
A token MAY implement both ERC-20 and this standard simultaneously, where ERC-20 functions operate on a separate "public balance" pool and encrypted functions operate on the "shielded balance" pool. Shield/unshield functions can move tokens between pools:
...
Implementations SHOULD use constant-size proofs and constant gas consumption for all transfer amounts. The STYX Protocol achieves this via uniform circuit compilation (all operations compile to the same gate count).
...
The contract's FHE secret key is the most sensitive asset. If compromised, all balances are exposed. Implementations MUST use threshold key management (Shamir Secret Sharing with ≥ 5 custodians) or hardware security modules (HSMs). The STYX Protocol extends this with the Oblivion ceremony (see ERC-YYYY: Cryptographic Amnesia Interface) for provable key destruction.
